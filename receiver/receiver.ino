...
struct ADPCMState {
  ...
  void softReset() {
    ...
    if (index > 60) index = 50;
    else if (index < 20) index = 30;

}
};

ADPCMState decoderState;
...
public:
  SimpleDCBlocker()
    : lastInput(0), lastOutput(0), alpha(0.999f) {}
...
public:
  ...
  int16_t process(int16_t input) {
    float x = (float)input;
    float y = x - lastInput + alpha * lastOutput;
...
public:
  SimpleTwoPoleLowPass() {

    float sampleRate = 8000.0f;
    float cutoffFreq = 3000.0f;  
  ...
  void setCutoff(float cutoffHz) {
    if (cutoffHz < 2000.0f) cutoffHz = 2000.0f;  
    if (cutoffHz > 3800.0f) cutoffHz = 3800.0f;  
...
ImprovedDCBlocker dcBlocker;
SimpleTwoPoleLowPass audioFilter;
...
uint8_t decodeADPCM(uint8_t code, ADPCMState* state) {
  ...
  return (uint8_t)((state->prevSample / 256) + 128);
}

void decompressAudioBlock(uint8_t* input, uint8_t* output, ADPCMState* state) {
  for (int i = 0; i < COMPRESSED_PACKET_SIZE; i++) {
    uint8_t code1 = input[i] & 0x0F;
    uint8_t code2 = (input[i] >> 4) & 0x0F;

    output[i * 2] = decodeADPCM(code1, state);
    output[i * 2 + 1] = decodeADPCM(code2, state);
  }
...
void setup() {
  ...
  audioDisplay.drawString(64, 30, "ADPCM RX");
  ...
  Serial.println("\n=== LoRa Audio Receiver - ADPCM 8kHz ===");
  Serial.printf("Device ID: 0x%04X\n", myDeviceId);
  Serial.printf("Sample Rate: %d Hz\n", SAMPLE_RATE);
  Serial.printf("ADPCM: 4-bit with gentle filtering\n");
  Serial.printf("Simplified audio processing\n\n");

  if (isPaired) {
    Serial.printf("Paired with transmitter: 0x%04X\n", pairedDeviceId);
    Serial.println("Ready to receive ADPCM audio\n");
  } else {
...
void watchdogTask(void* parameter) {
  ...
  while (true) {
    ...
    if (now - lastWatchdog > WATCHDOG_TIMEOUT) {
      ...
      decoderState.softReset();
      audioFilter.softReset();
...
void initI2S() {
  ...
  Serial.println("I2S initialized for 8kHz ADPCM audio");
}

void audioOutputTask(void* parameter) {
  Serial.println("Audio output task started - 8kHz with safe two-pole filtering");
  ...
  while (true) {
    ...
    if (!audioPlaying && currentLevel >= bufferParams.startThreshold) {
      ...
      Serial.println("▶️  Playback starting (8kHz with improved filtering)");
      lastPlayTime = now;
    }
...
void packetProcessorTask(void* parameter) {
  ...
  while (true) {
    if (xQueueReceive(audioPacketQueue, &audioPacket, portMAX_DELAY) == pdTRUE) {
      ...
      decompressAudioBlock(audioPacket.compressedData, decompressedBuffer, &decoderState);
...
void printAdaptiveStatus() {
  Serial.println("\n╔════════════════════════════════════════╗");
  Serial.println("║   8kHz ADAPTIVE STATUS               ║");
  Serial.println("╠════════════════════════════════════════╣");
...
void printStatusReport() {
  Serial.println("\n╔════════════════════════════════════════╗");
  Serial.println("║    8kHz RECEIVER STATUS              ║");
  Serial.println("╠════════════════════════════════════════╣");
...
void updateDisplay() {
  ...
  if (isPaired) {
    audioDisplay.setTextAlignment(TEXT_ALIGN_CENTER);
    audioDisplay.drawString(64, 0, "8kHz");
  }
  ...
  if (pairingMode) {
    ...
    if (level > 0) {
      ...
      audioDisplay.setFont(ArialMT_Plain_10);
      audioDisplay.drawString(64, 40, "Waiting 8kHz");
    }
  } else {
    audioDisplay.setFont(ArialMT_Plain_16);
